...
import { Konseling, Panggilortu, Siswa } from "@prisma/client";
...
import { LaporanKonseling } from "../../../../../components/Surat/LaporanJadwalKonseling";
...
export const getServerSideProps: GetServerSideProps<{
  ...
  const data = await prismaFront.konseling.findMany({
    where: {
      status: "Disetujui",
    },
    include: {
  ...
  if (!data) {
    return {
      redirect: {
        destination: "/admin/layanan/bimbingan",
        permanent: false,
      },
    };
  }
...
const RekapitulasiSetuju: NextPage<
...
> = ({ data }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  return (
    <AdminLayout
      title="Laporan Jadwal Konseling"
      breadcrumb={
        <Breadcrumb>
          <BreadcrumbItem>
            <BreadcrumbLink>Layanan</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbItem>
            <Link href={"/admin/layanan/bimbingan"}>
              <BreadcrumbLink>Bimbingan</BreadcrumbLink>
            </Link>
          </BreadcrumbItem>
        </Breadcrumb>
      }
    >
      <>
        <Head>
          <title>SMABAT || Laporan Jadwal Konseling</title>
        ...
        <div className="px-5 md:px-20 py-2 pb-5">
          ...
          <div className="p-5 md:p-10 min-w-[800px] overflow-auto my-5 bg-white rounded shadow">
            <LaporanKonseling data={data} ref={componentRef} />
          </div>
          {/* </Scrollbars>} */}
          <Button
            onClick={handlePrint}
...
export default RekapitulasiSetuju;
