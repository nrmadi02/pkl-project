...
import {
  createSiswaSchema,
  CreateSiswaSchema,
} from "../../../server/schema/siswa.schema";
...
const Pelanggaran: NextPage = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.useQuery(["siswa.getAll", selectKelas], {
  ...
  const data = useMemo(
    () => getData(dataSiswa?.result ? dataSiswa.result : []),
    [dataSiswa]
  );
    ...
    <AdminLayout
      ...
      breadcrumb={
        <Breadcrumb>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink href="#">Terlambat</BreadcrumbLink>
          </BreadcrumbItem>
        </Breadcrumb>
      }
    >
    ...
    <div className="flex flex-row items-center gap-x-[10px]">
      ...
      <NextLink href={`/admin/data/siswa/${data.id}`}>
        <div className="group flex cursor-pointer flex-col gap-y-[5px]">
          <p className="font-bold group-hover:text-orange-400 transition-all group-hover:scale-105 group-active:scale-100">
            {data.nama}
          </p>
          <p>{data.email}</p>
        </div>
        ...
        <div className="pt-5">
          ...
          <div className="flex flex-row gap-5">
            ...
            <div className="flex gap-3 flex-col text-[18px]">
              <tr>
                <td width={60} className="font-bold">
                  Nama
                </td>
                <td width={20}>:</td>
                <td>{data.nama}</td>
